import sys
import os #libreria para el erro cuDNN not initialize 
import numpy as np
...
os.environ['TF_FORCE_GPU_ALLOW_GROWTH'] = 'true' #Agregamos esto al codigo 	
...
def step_decay_schedule(initial_lr=0.0001, decay_factor=0.5, step_size=20):
    '''
...
def do_2stage_training(model, org_model, train_generator, validation_set, 
                       validation_steps, best_model_out, steps_per_epoch, 
                       top_layer_nb=10, nb_epoch=10, all_layer_epochs=0,
                       optim='sgd', init_lr=.01, all_layer_multiplier=.1,
                       es_patience=5, lr_patience=1, auto_batch_balance=True, 
                       nb_class=2,
                       pos_cls_weight=1., neg_cls_weight=1., nb_worker=1,
    ...
    #                                   patience=lr_patience, epsilon=1e-04, verbose=1)
                                    
        
    #     callbacks.append(reduce_lr)
    #Add the PrintLR callback here
...
class PrintLR(Callback):
   def on_epoch_end(self, epoch, logs=None):
        ...
        # Fetch the value within the TensorFlow variable
        if hasattr(lr, 'numpy'):
            lr = lr.numpy()
...
class DMAucModelCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        #     lr = float(K.get_value(self.model.optimizer.lr))
            # print(f"\nEpoch {epoch + 1}: Current learning rate is {lr}")
